<template>
  <div>
    ...
    <n-modal v-model:show="showModal" preset="dialog" :title="editItem ? 'Редактировать' : 'Добавить'" positive-text="Сохранить" negative-text="Отмена" @positive-click="save" style="width: 450px">
      <n-form :model="form">
        <n-form-item label="Гос. номер"><n-input v-model:value="form.plate_number" /></n-form-item>
...
const form = reactive({ plate_number: '', is_active: true })

const columns = [
  { title: '№', key: 'index', width: 60, render: (_: any, index: number) => index + 1 },
  { title: 'Гос. номер', key: 'plate_number' },
  ...
  { title: 'Действия', key: 'actions', width: 120, render: (row: any) => auth.isAdmin ? h(NButton, { size: 'small', onClick: () => { editItem.value = row; form.plate_number = row.plate_number; form.is_active = row.is_active; showModal.value = true } }, () => 'Ред.') : null },
]

async function load() { loading.value = true; try { items.value = (await api.get('/vehicles')).data } catch {} loading.value = false }
async function save() {
  try {
    if (editItem.value) { await api.put(`/vehicles/${editItem.value.id}`, form) } else { await api.post('/vehicles', form) }
    msg.success('Сохранено'); showModal.value = false; editItem.value = null; form.plate_number = ''; form.is_active = true; await load()
